...
namespace FPBooster.Plugins
{
    public partial class AutoRestockView : UserControl, IPlugin
    {
        ...
        // КЭШ ДАННЫХ: OfferID -> (Text, Quantity)
        // Запоминает, что вы вводили для каждого лота
        ...
        public void InitNodes(IEnumerable<string> nodes, string goldenKey)
        {
            ...
            // ИСПРАВЛЕНИЕ: Принудительно очищаем офферы при входе/обновлении нод
            ...
            {
                // Если список пуст - сбрасываем всё
            ...
            // Если нода выбрана, грузим офферы
        ...
        private async void NodeCombo_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            {
                OfferCombo.ItemsSource = null; // Очищаем офферы если нет ноды
                return;
            }
            await LoadOffersAsync(node);
        }

        // Обработчик смены оффера (для восстановления данных из кэша)
        private void OfferCombo_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            {
                // Если для этого лота уже были введены данные, восстанавливаем их
                ...
                {
                    // Иначе сброс (или можно оставить пустым)
                    ItemInput.Text = "";
                    QtyInput.Text = "1";
                }
            }
        ...
        private async Task LoadOffersAsync(string node)
        {
            ...
            try
            {
                // Отписываемся перед очисткой, чтобы не триггерить SelectionChanged
                ...
                {
                    ShowToast("Офферы не найдены в этом разделе");
                }
                else
                {
                    OfferCombo.ItemsSource = lots;
                    OfferCombo.DisplayMemberPath = "Title"; // Для старого стиля, в новом Template это игнорируется, но оставим
                    OfferCombo.SelectionChanged += OfferCombo_SelectionChanged;
            ...
            {
                AppendPluginLog($"[ERR] Ошибка загрузки: {ex.Message}");
            }
            finally
            {
        ...
        private void ItemInput_TextChanged(object sender, TextChangedEventArgs e) 
        {
            SaveToCache();
            UpdatePreview();
        }
        ...
        {
            if (!int.TryParse(QtyInput.Text, out _))
            {
                if (!string.IsNullOrEmpty(QtyInput.Text)) QtyInput.Text = "1";
            }
            SaveToCache();
            UpdatePreview();
        }

        // Сохраняем текущий ввод в кэш для текущего лота
        private void SaveToCache()
        {
            if (OfferCombo.SelectedItem is LotInfo lot)
            {
                _offerDataCache[lot.OfferId] = (ItemInput.Text, QtyInput.Text);
            }
        }

        private void BtnMinus_Click(object sender, RoutedEventArgs e)
        {
            if (int.TryParse(QtyInput.Text, out int val))
            {
                if (val > 1) QtyInput.Text = (val - 1).ToString();
            }
            else QtyInput.Text = "1";
        }
        ...
        private async void OnSaveClick(object sender, RoutedEventArgs e)
        {
            ...
            if (lot == null || string.IsNullOrEmpty(nodeId))
            {
                ShowToast("Выберите раздел и оффер!");
                return;
            }
            ...
            {
                ShowToast("Введите текст выдачи!");
                return;
            }
            ...
            if (sender is Button btn)
            {
                ...
                try 
                {
                    ...
                    if (ok)
                    {
                        AppendPluginLog($"[SUCCESS] Лот успешно обновлён!");
                        ShowToast("Сохранено!", true);
                    }
                    else
                    {
                        AppendPluginLog($"[ERR] Не удалось сохранить лот.");
                        ShowToast("Ошибка FunPay");
                    }
                }
                catch (Exception ex)
                {
                    AppendPluginLog($"[ERR] Исключение: {ex.Message}");
                }
                finally
                {
                    btn.IsEnabled = true;
                    btn.Content = "💾 Сохранить настройки";
                }
            }
        }
